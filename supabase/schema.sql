-- Supabase Schema for Song2Meaning Open Source Project
-- This file contains the complete database schema including tables, RLS policies, and indexes
...
DROP POLICY IF EXISTS "Public read access" ON song_analyses;
DROP POLICY IF EXISTS "Anonymous insert" ON song_analyses;
DROP POLICY IF EXISTS "Anonymous update access_count" ON song_analyses;
DROP POLICY IF EXISTS "Authenticated full access" ON song_analyses;
...
-- Create a function to normalize text for search
...
-- Verification queries (comment these out when running in production)
...
-- INSERT INTO song_analyses (share_id, artist, title, normalized_artist, normalized_title, analysis_data)
-- VALUES (
--     generate_share_id(),
--     'The Beatles',
--     'Hey Jude',
--     normalize_text('The Beatles'),
--     normalize_text('Hey Jude'),
--     '{"themes": ["love", "encouragement"], "mood": "uplifting", "summary": "A song about comfort and encouragement"}'
-- );
